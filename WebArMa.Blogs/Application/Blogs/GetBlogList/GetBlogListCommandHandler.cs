...
{
    public class GetBlogListCommandHandler(IWebArMaDbContext dbContext, WebArMaTypeAdapterConfig config) : IWebArMaCommandHandler<GetBlogListCommand, PaginatedItems<BlogDto>>
    {
        public async ValueTask<PaginatedItems<BlogDto>> Handle(GetBlogListCommand request, CancellationToken cancellationToken)
        {
            ...
            return new PaginatedItems<BlogDto>(blogs, totalCount, request.PageNumber, request.PageSize);
        }
    }
}
