...
namespace WebArMa.Blogs.Application.Blogs.GetBlugBySlug
{
    ...
    {
        public async ValueTask<BlogDto> Handle(GetBlogBySlugQuery query, CancellationToken cancellationToken)
        {
            return await webArMaDbContext
                .Set<Blog>()
                .ProjectToType<BlogDto>(config)
                .FirstOrDefaultAsync(b => b.Slug == query.Slug, cancellationToken: cancellationToken) ?? throw new NotFoundException("مطلب یافت نشد");
        }
    }
}
